...
def build_network(image, layers, variables):
    ...
    def _conv_to_output(mobile_net_output, output_layer_name):
        w = tf.nn.conv2d(mobile_net_output, _weights(output_layer_name),
    ...
    def _conv(inputs, stride, block_id):
        return tf.nn.relu6(
            tf.nn.conv2d(inputs, _weights("Conv2d_" + str(block_id)), stride,
                         padding='SAME')
    ...
    def _separable_conv(inputs, stride, block_id, dilations):
        ...
        w = tf.nn.depthwise_conv2d(
            inputs, _depthwise_weights(dw_layer), stride, 'SAME',
            rate=dilations, data_format='NHWC')
...
def convert(model_id, model_dir, check=False):
    ...
    with cg.as_default():
        ...
        with tf.Session() as sess:
            ...
            freeze_graph(
                input_graph=os.path.join(model_dir,
